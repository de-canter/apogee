...
- [ ] **Approve the B4 PR** (`feature/b4-documents-knowledge`), then tag `documents-ai-v0.1.0` and `knowledge-v0.1.0` and confirm both releases carry their tarballs.
...
2. **B4** `@apogee/documents-ai` + `@apogee/knowledge` — packages built on `feature/b4-documents-knowledge` (plan `docs/plans/2026-09-19-b4-documents-knowledge.md`); PR to approve, then tag `documents-ai-v0.1.0` and `knowledge-v0.1.0`; the document and knowledge demos follow in apogee-build.
